...
public class SubjectSelection extends JPanel {
    ...
    private MainApplication mainApp;
    private StudentData student;
    private JLabel noSubjectsLabel;
    private DataStore.SubjectData selectedSubject; // Store the selected subject

    public SubjectSelection(MainApplication mainApp, StudentData student) {
        this.mainApp = mainApp;
        this.student = student;
        this.selectedSubject = null; // Initialize to null
        setLayout(new BorderLayout());
        setBackground(new Color(200, 210, 220));
        initializeComponents();
    ...
    private void handleSelect() {
        int selectedRow = subjectTable.getSelectedRow();
        if (selectedRow >= 0) {
            // Fetch the subject data from DataStore based on the row data
            int edpCode = (Integer) tableModel.getValueAt(selectedRow, 0);
            for (DataStore.SubjectData subject : DataStore.getSubjects()) {
            ...
            String subjectName = (String) tableModel.getValueAt(selectedRow, 3);
            JOptionPane.showMessageDialog(this, "Selected subject: " + subjectName, "Success", JOptionPane.INFORMATION_MESSAGE);
        } else {
            this.selectedSubject = null; // Clear selection if no row is selected
            JOptionPane.showMessageDialog(this, "No subject selected.", "Selection Error", JOptionPane.WARNING_MESSAGE);
        }
    }

    // Method to get the selected subject
    public DataStore.SubjectData getSelectedSubject() {
